...
struct GoogleCredentials: Codable {
    ...
    /// Search directories in priority order: app-owned credentials first, then MCP sources.
    static func discover() -> (credentials: GoogleCredentials, email: String, source: String)? {
        let home = FileManager.default.homeDirectoryForCurrentUser
        let searchDirs: [(URL, String)] = [
            (home.appendingPathComponent(".boomi-sre/credentials"), "~/.boomi-sre/credentials/"),
            (home.appendingPathComponent(".google_workspace_mcp/credentials"), "~/.google_workspace_mcp/credentials/"),
            (home.appendingPathComponent(".amazonq/mcp_credentials"), "~/.amazonq/mcp_credentials/"),
            (home.appendingPathComponent(".kiro/mcp_credentials"), "~/.kiro/mcp_credentials/"),
        ]
        for (dir, label) in searchDirs {
